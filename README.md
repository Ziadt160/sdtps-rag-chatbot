...
[![CI](https://github.com/Ziadt160/sdtps-rag-chatbot/actions/workflows/ci.yml/badge.svg)](https://github.com/Ziadt160/sdtps-rag-chatbot/actions/workflows/ci.yml)
...
ruff check .        # lint
pytest              # unit tests (arabic, parse, chunk, RRF, cache)
```

[GitHub Actions](.github/workflows/ci.yml) runs on every push and pull request to `main`:

- **Lint & test** — `ruff` + `pytest` on Python 3.12.
- **Build & publish** — on push to `main` (and `v*` tags) builds the Docker image and pushes
  it to GHCR (`ghcr.io/ziadt160/sdtps-rag-chatbot`) using buildx layer caching, tagged
  `latest`, the commit SHA, branch, and semver for tags.

The **CI badge** at the top links to the latest run.
